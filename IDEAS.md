...
- **CI/CD com GitHub Actions self-hosted runner** 🟡 *(parcialmente feito em 2026-08-18)* — runner self-hosted instalado como serviço systemd no próprio servidor k3s, registrado no repo; workflow em `.github/workflows/deploy.yml` builda a imagem, importa no containerd e aplica os manifests, sem precisar de `rsync`/`ssh` pra outro host (detalhes de instalação/operação em `ops/DEPLOY-k3s.md`, seção CI/CD). **Falta:** hoje o gatilho é manual (`workflow_dispatch` via Actions → Run workflow) — decidiu-se validar mais alguns runs antes de trocar para `push` automático na `main`. `ops/deploy.sh` continua valendo como alternativa manual.
